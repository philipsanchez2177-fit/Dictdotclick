...
The **Xcode project exists and is committed.** Phase 0 is built and verified on the Mac. Phase 1
is written but **not yet compiled**.
...
| `Dictdotclick/UI/Settings/` | **Written, not compiled.** Phase 1 — sidebar-tabbed Settings window, seven files. |

**Phase 0 complete; Phase 1 written.** The project file was authored directly rather than generated by Xcode's New
Project wizard, so Philip's steps are `git pull` and ⌘R (see `RUN-IT.md`). Verified on the Mac
2026-08-19: `BUILD SUCCEEDED`, app launches, mic icon in the menu bar, no Dock or ⌘-Tab entry.
**Next: build Phase 1 on the Mac, then Phase 2.**
...
| 1 | `MenuBarExtra` + Settings window with Liquid Glass and sidebar tabs. Skeleton only. | **Written, not compiled** — 2026-08-19 |
